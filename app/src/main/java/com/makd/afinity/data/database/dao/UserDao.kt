...
interface UserDao {
    ...
    @Query("DELETE FROM users WHERE id = :userId") suspend fun deleteUserById(userId: UUID)

    @Query("DELETE FROM users WHERE serverId = :serverId")
    suspend fun deleteUsersByServerId(serverId: String)

    @Query("SELECT * FROM users WHERE id = :userId") suspend fun getUser(userId: UUID): User?
