...
interface MovieDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insertMovie(movie: AfinityMovieDto)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertMovies(movies: List<AfinityMovieDto>)

    @Update suspend fun updateMovie(movie: AfinityMovieDto)

    @Delete suspend fun deleteMovie(movie: AfinityMovieDto)

    @Query("DELETE FROM movies WHERE id = :movieId") suspend fun deleteMovieById(movieId: UUID)
    ...
    @Query(
        "SELECT * FROM movies WHERE serverId = :serverId AND premiereDate >= :fromDate ORDER BY premiereDate DESC LIMIT :limit"
    )
    ...
    @Query("DELETE FROM movies") suspend fun deleteAllMovies()
}
