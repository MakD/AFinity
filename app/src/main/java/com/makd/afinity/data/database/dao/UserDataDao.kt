...
interface UserDataDao {
    ...
    @Query("DELETE FROM userdata WHERE userId = :userId AND itemId = :itemId AND serverId = :serverId")
    ...
    @Query("SELECT * FROM userdata WHERE userId = :userId AND itemId = :itemId AND serverId = :serverId")
    ...
    @Query("SELECT * FROM userdata WHERE userId = :userId AND serverId = :serverId AND favorite = 1")
    ...
    @Query("SELECT * FROM userdata WHERE userId = :userId AND serverId = :serverId AND playbackPositionTicks > 0 AND played = 0")
    ...
    @Query("SELECT * FROM userdata WHERE userId = :userId AND serverId = :serverId AND toBeSynced = 1")
    suspend fun getUnsyncedUserData(userId: UUID, serverId: String): List<AfinityUserDataDto>

    @Query("UPDATE userdata SET toBeSynced = 0 WHERE userId = :userId AND itemId = :itemId AND serverId = :serverId")
    suspend fun markUserDataSynced(userId: UUID, itemId: UUID, serverId: String)

    @Query("UPDATE userdata SET toBeSynced = 1 WHERE userId = :userId AND itemId = :itemId AND serverId = :serverId")
