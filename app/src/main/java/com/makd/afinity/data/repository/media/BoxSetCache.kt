...
class BoxSetCache @Inject constructor(
    ...
    private suspend fun ensureInitialized() = mutex.withLock {
        ...
        try {
            ...
            if (metadata != null) {
                ...
                Timber.d("Found cache metadata: lastBuild=${lastBuiltTimestamp}, version=${metadata.cacheVersion}")

                if (metadata.cacheVersion != currentCacheVersion) {
                    Timber.w("Cache version mismatch (${metadata.cacheVersion} != $currentCacheVersion), clearing cache")
                    clearDatabase()
                    isInitialized = true
                    return@withLock
                }
            ...
            entries.forEach { entry ->
                try {
                    ...
                    val boxSetIds = json.decodeFromString<List<String>>(entry.boxSetIds)
            ...
            Timber.i("Loaded BoxSet cache from database in ${duration}ms: ${loadedMap.size} items")
        } catch (e: Exception) {
    ...
    private suspend fun saveToDatabase(cacheMap: Map<UUID, List<UUID>>) {
        try {
            ...
            val entities = cacheMap.map { (itemId, boxSetIds) ->
                BoxSetCacheEntity(
            ...
            val metadata = BoxSetCacheMetadata(
                id = 1,
...
data class BoxSetWithChildren(
    val boxSetId: UUID,
    val childItemIds: List<UUID>
)
