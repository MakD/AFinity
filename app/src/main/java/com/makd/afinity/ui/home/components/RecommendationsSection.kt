        ...
        val title =
            when (section.sectionType) {
                PersonSectionType.STARRING ->
                    stringResource(R.string.home_person_starring, section.person.name)
                PersonSectionType.DIRECTED_BY ->
                    stringResource(R.string.home_person_directed_by, section.person.name)
    ...
    Column(modifier = modifier.padding(horizontal = 14.dp)) {
        ...
        LazyRow(
            ...
            items(
                ...
                key = { movie ->
                    "recommend_${section.sectionType.name}_${section.referenceMovie.id}_${movie.id}"
                },
            ) { movie ->
                MediaItemCard(item = movie, onClick = { onItemClick(movie) }, cardWidth = cardWidth)
            }
